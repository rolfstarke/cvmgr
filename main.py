...
parser.add_argument("--upload", help="use the pipeline.yaml to upload to roboflow", action='store_true')
...
try:
    ...
    if args.evaluate:
        for model_name in pipeline_yaml.get("models_to_evaluate", []):
            run_path = pathlib.Path.cwd() / "runs" / "segment" / "cvmgr" / model_name
            best = run_path / "weights" / "best.pt"
            label_field = model_name.replace("-", "_")
            try:
                evaluate_model(prediction_model=str(best), prediction_labelfield=label_field)
    ...
    if args.upload:
        for dataset in pipeline_yaml.get("datasets_to_upload", []):
